                            ...
                            <table class="table table-hover" id="jsdatatable_list">
                                ...
                                @foreach($getData as $item)
                                    <tr id="row-{{ $item->id }}">
                                        ...
                                        <td>
                                            <span class="badge text-white" style="background-color: {{ $item->header_color ?? '#0096a6' }}; px-3 py-2;">
                                                {{ $item->header_color ?? '#0096a6' }}
                                            </span>
                                        </td>
                                        <td>
                                            @if($item->badge)
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            $(document).on('change', '.toggle-status', function () {
                ...
                $.ajax({
                    url: '/admin/umrah-packages/' + id + '/status',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        status: status
                ...
                btn.addEventListener("click", async function () {
                    ...
                    try {
                        ...
                        const rows = `
                            ...
                            <tr><th>Subtitle</th><td>${data.subtitle ?? '-'}</td></tr>
                            <tr><th>Price</th><td>${data.currency ?? 'AED'} ${data.price ?? '-'}</td></tr>
                            <tr><th>Badge / Ribbon</th><td>${data.badge ?? '-'}</td></tr>
                            <tr><th>Header Color</th><td>${data.header_color ?? '-'}</td></tr>
                            <tr><th>Features</th><td>${data.features ? data.features.replace(/\\n/g, '<br>') : '-'}</td></tr>
                            <tr><th>Button Title</th><td>${data.button_title ?? '-'}</td></tr>
                            <tr><th>Button URL</th><td>${data.button_url ?? '-'}</td></tr>
