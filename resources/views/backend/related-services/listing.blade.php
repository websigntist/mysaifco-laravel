                                            ...
                                            <td class="capitalize">
                                                ...
                                                @elseif($col === 'title')
                                                    <div class="d-flex flex-column">
                                                        ...
                                                        @if($data->description)
                                                            <small class="text-muted">{{ Str::words(strip_tags($data->description), 10, '...') }}</small>
                                                        @endif
                                                    </div>
                                                @elseif($col === 'page_link')
                                                    @if($data->page_link)
                                                        <a href="{{ $data->page_link }}" target="_blank" class="text-primary text-decoration-underline text-break" style="max-width: 200px; display: inline-block;">
                                                            {{ Str::limit($data->page_link, 30) }}
                                                        </a>
                                                    @else
                                                        <span class="text-muted">—</span>
                                                    @endif
                                                @elseif($col === 'tour_type')
                                                    @php
                                                        $ids = is_array($data->tour_type_ids) ? $data->tour_type_ids : (json_decode($data->tour_type_ids ?? '[]', true) ?: []);
                                                        $names = collect($ids)->map(fn($id) => $tourTypeMap[$id] ?? null)->filter()->implode(', ');
                                                    @endphp
                                                    {{ !empty($names) ? $names : '—' }}
                                                @elseif($col === 'status')
            ...
            document.querySelectorAll(".viewBtn").forEach(btn => {
                btn.addEventListener("click", async function () {
                    ...
                    try {
                        ...
                        let linkRow = '';
                        if (data.page_link) {
                            linkRow = `<tr><th>Page Link</th><td><a href="${data.page_link}" target="_blank">${data.page_link}</a></td></tr>`;
                        }
