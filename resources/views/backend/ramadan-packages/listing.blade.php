                            ...
                            <table class="table table-hover" id="jsdatatable_list">
                                ...
                                @foreach($getData as $item)
                                    <tr id="row-{{ $item->id }}">
                                        ...
                                        <td>
                                            <span class="badge bg-label-primary fs-6">{{ $item->departure_day ?? '-' }}</span>
                                        </td>
                                        <td>
                                            <span class="badge bg-label-info fs-6">{{ $item->arrival_day ?? '-' }}</span>
                                        </td>
                                        <td>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            $(document).on('change', '.toggle-status', function () {
                ...
                $.ajax({
                    url: '/admin/ramadan-packages/' + id + '/status',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        status: status
                ...
                btn.addEventListener("click", async function () {
                    ...
                    try {
                        ...
                        const rows = `
                            ...
                            <tr><th>Month</th><td>${data.month ?? '-'}</td></tr>
                            <tr><th>Departure Day</th><td>${data.departure_day ?? '-'}</td></tr>
                            <tr><th>Departure Dates</th><td>${data.departure_dates ? data.departure_dates.replace(/\\n/g, '<br>') : '-'}</td></tr>
                            <tr><th>Arrival Day</th><td>${data.arrival_day ?? '-'}</td></tr>
                            <tr><th>Arrival Dates</th><td>${data.arrival_dates ? data.arrival_dates.replace(/\\n/g, '<br>') : '-'}</td></tr>
