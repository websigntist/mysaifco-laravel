                                            ...
                                            <td class="capitalize">
                                                ...
                                                @elseif($col === 'image')
                                                    <div class="avatar avatar-md lightgallery">
                                                        <a href="{{ $data->image ? asset('assets/images/'.$module .'/'. $data->image) : '#' }}">
                                                ...
                                                @elseif($col === 'friendly_url')
                                                    <code class="small">{{ $data->friendly_url }}</code>
            ...
            document.querySelectorAll(".viewBtn").forEach(btn => {
                btn.addEventListener("click", async function () {
                    ...
                    try {
                        ...
                        const imageUrl = data.image ? `/assets/images/{{$module}}/${data.image}` : "{{ imageNotFound() }}";
                        const rows = `
                            ...
                            <tr><th>{{_label('image')}}</th><td><img src="${imageUrl}" alt="" style="max-height:80px" class="rounded"></td></tr>
                            <tr><th>{{_label('status')}}</th><td><span class="badge ${data.status === 'Active' ? 'bg-label-success' : 'bg-label-danger'}">${data.status ? data.status : '-'}</span></td></tr>
