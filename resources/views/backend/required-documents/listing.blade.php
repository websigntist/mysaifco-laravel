                                            ...
                                            <td class="capitalize">
                                                ...
                                                @elseif($col === 'title')
                                                    <div class="d-flex flex-column">
                                                        ...
                                                        @if(is_array($data->document_items) && count($data->document_items))
                                                            <small class="text-muted"><i class="ti tabler-list-check icon-xs"></i> {{ count($data->document_items) }} Items</small>
                                                        @endif
                                                    </div>
                                                @elseif($col === 'subtitle')
                                                    <span class="text-muted">{{ Str::words($data->subtitle ?? '—', 8, '...') }}</span>
                                                @elseif($col === 'status')
                ...
                btn.addEventListener("click", async function () {
                    ...
                    try {
                        ...
                        if (Array.isArray(data.document_items) && data.document_items.length) {
                            itemsHtml = '<ol class="mb-0 ps-3">';
                            data.document_items.forEach(item => {
                                itemsHtml += `<li><strong>${item.title || ''}</strong>: ${item.description || ''}</li>`;
                            });
                            itemsHtml += '</ol>';
                        }
                        ...
                        const rows = `
                            ...
                            ${imageRow}
                            <tr><th>Main Title</th><td>${data.title ?? '-'}</td></tr>
                            <tr><th>Subtitle</th><td>${data.subtitle ?? '-'}</td></tr>
                            <tr><th>Document Items</th><td>${itemsHtml}</td></tr>
                            <tr>
                                <th>Status</th>
                                <td>
