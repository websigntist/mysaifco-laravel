...
@section('content')
    <div class="container-xxl flex-grow-1 mt-3 -container-p-y">
          <form action="{{route('invoices.update', $getData->id)}}" method="post" class="needs-validation" enctype="multipart/form-data" novalidate>
              @csrf
              @method('PUT')
             <div class="row gy-6">
                 <div class="col-sm-12">
                            ...
                            <div class="row g-6 pt-5">
                               <div class="col-md-3">
                                  ...
                                  <select class="form-select select2" id="invoice_type" name="invoice_type" required>
                                      ...
                                      @foreach($invoiceTypes as $key => $type)
                                          <option value="{{$key}}" {{ old('invoice_type', $getData->invoice_type ?? 'sales') == $key ? 'selected' : '' }}>
                                              {{$type}}
                                          </option>
                                      @endforeach
                                  </select>
                               ...
                               <div class="col-md-3">
                                  ...
                                  <input type="text"
                                         id="invoice_number"
                                         name="invoice_number"
                                         value="{{ old('invoice_number', $getData->invoice_number) }}"
                                         class="form-control"
                               ...
                               <div class="col-md-3">
                                  ...
                                  <select class="form-select select2" id="status" name="status" required>
                                      ...
                                      @foreach($getStatus as $status)
                                          <option value="{{$status}}" {{ old('status', $getData->status) == $status ? 'selected' : '' }}>
                                              {{ucfirst($status)}}
                                          </option>
                                      @endforeach
                                  </select>
                               ...
                               <div class="col-md-3">
                                  ...
                                  <select class="form-select select2" id="payment_status" name="payment_status" required>
                                      ...
                                      @foreach($paymentStatus as $pStatus)
                                          <option value="{{$pStatus}}" {{ old('payment_status', $getData->payment_status ?? 'Paid') == $pStatus ? 'selected' : '' }}>
                                              {{$pStatus}}
                                          </option>
                                      @endforeach
                                  </select>
                               ...
                               <div class="col-md-4">
                                  ...
                                  <select class="form-select select2" id="currency" name="currency" required>
                                      ...
                                      @foreach($currencies as $code => $symbol)
                                          <option value="{{$code}}" {{ old('currency', $getData->currency ?? 'USD') == $code ? 'selected' : '' }}>
                                              {{$code}} ({{$symbol}})
                                          </option>
                                      @endforeach
                                  </select>
                               ...
                               <div class="col-md-4">
                                  ...
                                  <input type="date"
                                         id="invoice_date"
                                         name="invoice_date"
                                         value="{{ old('invoice_date', $getData->invoice_date->format('Y-m-d')) }}"
                               ...
                               <div class="col-md-4">
                                  ...
                                  <input type="date"
                                         id="due_date"
                                         name="due_date"
                                         value="{{ old('due_date', $getData->due_date->format('Y-m-d')) }}"
                                         class="form-control dob-picker"
                                         placeholder="YYYY-MM-DD"
                                         required>
                            ...
                            <div class="row g-6 pt-5">
                               <div class="col-md-12">
                                  ...
                                  <input type="text"
                                         id="client_name"
                                         name="client_name"
                                         value="{{ old('client_name', $getData->client_name) }}"
                                         class="form-control"
                               ...
                               <div class="col-md-6">
                                  ...
                                  <input type="email"
                                         id="client_email"
                                         name="client_email"
                                         value="{{ old('client_email', $getData->client_email) }}"
                                         class="form-control"
                               ...
                               <div class="col-md-6">
                                  <label class="form-label text-capitalize" for="client_phone">
                                      <span>{{_label('client_phone')}}</span>
                                  </label>
                                  <input type="text"
                                         id="client_phone"
                                         name="client_phone"
                                         value="{{ old('client_phone', $getData->client_phone) }}"
                                         class="form-control"
                               ...
                               <div class="col-md-12">
                                  ...
                                  <textarea class="form-control"
                                            ...
                                            rows="3">{{ old('client_address', $getData->client_address) }}</textarea>
                               </div>
                            </div>
                         </div>
                      </div>
                      ...
                      <div class="collapse show">
                         <div class="card-body">
                            <div id="invoice-items-container">
                                ...
                                <div class="invoice-item border p-3 mb-3 rounded" data-item="{{$index}}">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <h6 class="mb-0 text-primary">Item #<span class="item-counter">{{ $index + 1 }}</span></h6>
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <textarea name="items[{{$index}}][description]"
                                                      ...
                                                      rows="2">{{ old('items.'.$index.'.description', $item->description) }}</textarea>
                                        </div>
                                        <div class="col-md-3">
                            ...
                            <div class="row g-6 pt-5">
                               <div class="col-md-12">
                                  ...
                                  <textarea class="form-control"
                                            ...
                                            rows="3">{{ old('notes', $getData->notes) }}</textarea>
                               </div>
                               <div class="col-md-12">
                                  ...
                                  <textarea class="form-control"
                                            ...
                                            rows="3">{{ old('terms', $getData->terms) }}</textarea>
                               </div>
                            </div>
                         </div>
                      </div>
                               ...
                               <div class="col-md-4">
                                  ...
                                  @if($getData->letterhead)
                                      <div class="mb-2 position-relative d-inline-block" id="letterhead-preview">
                                          <img src="{{ asset('assets/images/inv-assets/' . $getData->letterhead) }}"
                                               alt="Letterhead"
                            ...
                            <div class="row g-4">
                               <div class="col-12">
                                   <div class="d-flex justify-content-between align-items-center text-danger mb-3 mt-5">
                                       ...
                                       <strong id="subtotal-display">${{ number_format($getData->subtotal, 2) }}</strong>
                                   </div>
                                   <input type="hidden" name="subtotal" id="subtotal" value="{{ $getData->subtotal }}">
                               </div>
                               <div class="col-12">
                               ...
                               <div class="col-12">
                                  ...
                                  <input type="number"
                                         ...
                                         name="discount"
                                         class="form-control"
                                         value="{{ old('discount', $getData->discount) }}"
                                         step="0.01"
                                         min="0">
                               </div>
                                <div class="col-12">
                                    <div class="form-check form-switch mb-2">
                                        <input class="form-check-input" type="checkbox" id="show_tax" name="show_tax" value="1" {{ old('show_tax', $getData->show_tax) ? 'checked' : '' }}>
                                        <label class="form-check-label" for="show_tax">Show TAX With Total</label>
                                    </div>
                                    <div class="form-check form-switch mb-2">
                                        <input class="form-check-input" type="checkbox" id="show_vat" name="show_vat" value="1" {{ old('show_vat', $getData->show_vat) ? 'checked' : '' }}>
                                        <label class="form-check-label" for="show_vat">Show VAT With Total</label>
                                    </div>
                                    <div class="form-check form-switch mb-2">
                                        <input class="form-check-input" type="checkbox" id="show_discount" name="show_discount" value="1" {{ old('show_discount', $getData->show_discount) ? 'checked' : '' }}>
                               ...
                               <div class="col-12">
                                   <div class="d-flex justify-content-between align-items-center">
                                       ...
                                       <h5 class="text-danger fw-semibold fs-4" id="total-display">
                                           ${{ number_format ($getData->total, 2) }}
                                       </h5>
                                   </div>
                                   <input type="hidden" name="total" id="total" value="{{ $getData->total }}">
                               </div>
                            </div>
                         </div>
                      </div>
...
$(document).ready(function() {
    let itemCount = {{ count($getData->items) }};
