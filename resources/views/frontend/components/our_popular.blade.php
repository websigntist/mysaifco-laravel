            ...
            <div class="our-popular-grid mt-8 grid grid-cols-1 gap-4 sm:gap-5 md:mt-10 md:grid-cols-3 md:gap-5">
                @foreach ($popular_categories as $index => $category)
                    ...
                    <article @class([
                        ...
                        <a href="{{ $url }}" class="our-popular-card__link">
                            <img
                                src="{{ $imageSrc }}"
                ...
                @foreach ($popular_categories as $index => $category)
                    @php
                        ...
                        $imageSrc = str_contains($category->image ?? '', '/')
                            ...
                            : ($category->image ? asset('assets/images/tour-types/' . $category->image) : asset('assets/images/product-category/01.webp'));
                    @endphp
                    <div class="relative {{ $index === 1 ? 'col-span-2' : '' }}">
                        <a href="{{ $url }}">
                            <img src="{{ $imageSrc }}"
                            ...
                            <div class="absolute left-5 bottom-8 font-heading text-xl font-medium text-white z-10">{{ $category->title }}</div>
                            <div class="absolute left-5 bottom-3 font-body text-xs text-white/70 z-10">{{ $category->tours_count }} {{ Str::plural('Tour', $category->tours_count) }}</div>
                        </a>
                    </div>
                @endforeach
            </div>
