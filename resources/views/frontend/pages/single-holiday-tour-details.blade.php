...
<section class="pt-6">
    <div class="container mx-auto">
        <nav class="flex items-center gap-2 text-sm font-heading" aria-label="Breadcrumb">
            <a href="{{ url('/') }}" class="text-mst-gray hover:text-mst transition">Home</a>
            ...
            <a href="{{ url('/holiday-packages') }}" class="text-mst-gray hover:text-mst transition">Holiday
                                                                                                     Packages</a>
            <svg class="w-3.5 h-3.5 text-mst" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" d="m9 5 7 7-7 7"/>
            </svg>
            ...
            <div>
                    ...
                    @foreach($facts as $fact)
                        <div class="flex flex-col gap-1">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="font-body text-xs -mt-1 text-gray-500">{{ $fact['label'] }}</span>
                            </div>
                            <span class="font-heading font-bold text-xs text-mst-gray ms-8 -mt-2">{{ $fact['value'] }}</span>
                        </div>
                    @endforeach
                </div>
                {{-- Highlights --}}
                <div class="grid grid-cols-2 md:grid-cols-5 gap-3 mt-6">
                    @foreach($highlights as $highlight)
                        <div class="flex flex-col items-center justify-center text-center gap-2 bg-gray-50 border border-gray-200 rounded-xl py-4 px-2">
                            <img src="{{ asset('assets/images/icons/' . $highlight['icon']) }}" alt="">
                            <span class="font-heading italic font-bold text-sm text-mst-gray">{{ $highlight['label'] }}</span>
                        </div>
                    @endforeach
                </div>
                {{-- Itinerary --}}
                    ...
                    @foreach($itinerary as $item)
                        <div class="flex items-start gap-4">
                            <div class="flex flex-col items-center justify-center w-12 h-12 flex-shrink-0 rounded-full bg-gradient-to-r from-[#BA9B31] to-[#74611E] text-white leading-none">
                                ...
                                <span class="font-heading font-bold text-base">{{ $item['day'] }}</span>
                            </div>
                            <div>
                                <h3 class="font-heading font-bold text-lg no-italic text-mst-gray">{{ $item['title'] }}</h3>
                                <p class="font-body text-sm text-mst-gray mt-1">{{ $item['desc'] }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mt-10">
                        ...
                        <ul class="space-y-3">
                            @foreach($included as $inc)
                                <li class="flex items-start gap-3">
                                    ...
                                    <span class="font-body text-sm text-mst-gray">{{ $inc }}</span>
                                </li>
                            @endforeach
                        </ul>
                    </div>
                    <div class="bg-gray-50 border border-gray-200 rounded-2xl p-6">
                        ...
                        <ul class="space-y-3">
                            @foreach($notIncluded as $exc)
                                <li class="flex items-start gap-3">
                                    <img src="{{ asset('assets/images/icons/777.svg') }}" alt="">
                                    <span class="font-body text-sm text-mst-gray">{{ $exc }}</span>
                                </li>
                            @endforeach
                        </ul>
                    </div>
                ...
                <form id="detailed-booking-form" class="space-y-6">
                    ...
                    <div class="space-y-3">
                        <button type="submit" class="w-full flex items-center justify-center gap-2 rounded-full
                            ...
                            <img src="{{ asset('assets/images/icons/lock.svg') }}" alt=""> Book Now
                        </button>
    ...
    <div class="container mx-auto">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="rounded-2xl overflow-hidden h-64 lg:h-auto">
                <img src="{{ asset('assets/images/packages/989898.webp') }}" alt="3 Star Hotel" class="w-full h-full object-cover">
            </div>
            {{-- Description --}}
            <div class="bg-gray-50 border border-gray-200 rounded-2xl p-6">
                ...
                @endphp
                <ul class="space-y-5">
                    @foreach($hotelPoints as $point)
                        <li class="flex items-center gap-3">
                            <img src="{{ asset('assets/images/icons/' . $point['icon']) }}" alt="">
                            <span class="font-body text-sm text-mst-gray">{{ $point['label'] }}</span>
                        </li>
                    @endforeach
                </ul>
            </div>
                ...
                <div class="grid grid-cols-3 gap-3">
                    @foreach($amenities as $amenity)
                        <div class="flex flex-col items-center justify-center text-center gap-2 bg-white border border-gray-200 rounded-xl py-4 px-1">
                            <img src="{{ asset('assets/images/icons/' . $amenity['icon']) }}" alt="">
                            <span class="font-body text-[11px] text-mst-gray leading-tight">{{ $amenity['label'] }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
        </div>
        {{-- Trusted partner note --}}
        <div class="bg-[#FAF7F2] border border-[#BA9B31]/30 rounded-xl p-4 mt-6 flex items-center justify-center gap-3 text-center">
            <img src="{{ asset('assets/images/icons/8789.svg') }}" class="w-6" alt="">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-10">
            @foreach($features as $feature)
                <div class="grid grid-cols-[auto_1fr] gap-4 px-6 items-center">
                    <div>
                        <img src="{{ asset('assets/images/icons/' . $feature['icon']) }}"
                             class="w-10 h-10 object-contain" alt="">
                    </div>
                    <div>
                        <h4 class="font-heading italic font-bold text-base text-mst-gray">{{ $feature['title'] }}</h4>
                        <p class="font-body text-xs text-mst-gray mt-1 leading-snug">{{ $feature['desc'] }}</p>
                    </div>
                </div>
            @endforeach
        </div>
...
{{-- ===== Important Notes + Popular Services ===== --}}
<section class="pt-14">
    <div class="container mx-auto">
                    ...
                    @foreach($popularServices as $service)
                        <article class="bg-white border border-gray-200 rounded-2xl overflow-hidden flex flex-col">
                            <div class="relative h-40">
                                <img src="{{ asset('assets/images/packages/' . $service['image']) }}" alt="{{ $service['title'] }}" class="w-full h-full object-cover">
                                <span class="absolute bottom-3 left-3 bg-gray-800 text-white text-xs font-heading
                                italic px-3 py-1 rounded-md">{{ $service['badge'] }}</span>
                            </div>
                            <div class="p-4 flex flex-col flex-1">
                                <h4 class="font-heading font-bold text-sm text-mst-gray leading-snug line-clamp-1">{{ $service['title'] }}</h4>
                                ...
                                <p class="font-body text-xs text-mst-gray mt-2 leading-snug">{{ $service['desc'] }}</p>
                                <div class="flex items-end justify-between mt-auto pt-2">
                                    <div>
                                        <p class="font-body text-xs text-mst-gray">Starting from</p>
                                        <p class="font-heading font-bold text-mst">AED {{ $service['price'] }}</p>
                                    </div>
                                    <a href="#" class="w-9 h-9 rounded-full bg-gradient-to-r from-[#BA9B31] to-[#74611E] text-white flex items-center justify-center hover:opacity-90 transition" aria-label="View {{ $service['title'] }}">
        ...
        <div class="bg-[#FAF7F2] border border-[#BA9B31]/30 rounded-xl p-4 mt-8 flex items-center justify-center gap-3 text-center">
            <img src="{{ asset('assets/images/icons/46.svg') }}" alt="">
