...
@section('content')
    @php
        /*
        |--------------------------------------------------------------------------
        | Hide Page Top Banner Array
        |--------------------------------------------------------------------------
        | Add page slugs or IDs into this array to hide the top banner section
        | Example: ['umrah-by-air', 'umrah-by-bus', 'contact-us']
        |
        */
        $hideBannerPages = [
            // Add page slugs here to hide top banner, e.g.:
            'umrah-for-single-lady',
            // 'umrah-by-air',
        ];

        $currentSlug = strtolower($page->friendly_url ?? $page->slug ?? request()->route('slug') ?? '');
        $currentId   = $page->id ?? null;

        $showTopBanner = !in_array($currentSlug, array_map('strtolower', $hideBannerPages))
                      && !in_array($currentId, $hideBannerPages);
