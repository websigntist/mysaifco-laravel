...
return new class extends Migration
{
    ...
    {
        $exists = DB::table('modules')->where('module_slug', 'umrah-bus-schedules')->exists();
        if (!$exists) {
            $maxOrder = DB::table('modules')->max('ordering') ?? 0;

            $data = [
                'parent_id'     => 0,
                'module_title'  => 'Umrah Bus Schedule',
                'module_slug'   => 'umrah-bus-schedules',
                'status'        => 'Active',
                'ordering'      => $maxOrder + 1,
                'created_by'    => 1,
                'created_at'    => now(),
            ...
            if (Schema::hasColumn('modules', 'icon')) {
                $data['icon'] = 'tabler-calendar-event';
            }
    ...
    {
        $module = DB::table('modules')->where('module_slug', 'umrah-bus-schedules')->first();
        if ($module) {
