...
return new class extends Migration
{
    ...
    {
        $exists = DB::table('modules')->where('module_slug', 'ramadan-packages')->exists();
        if (!$exists) {
            $maxOrder = DB::table('modules')->max('ordering') ?? 0;

            $data = [
                'parent_id'     => 0,
                'module_title'  => 'Ramadan Packages',
                'module_slug'   => 'ramadan-packages',
                'status'        => 'Active',
                'ordering'      => $maxOrder + 1,
                'created_by'    => 1,
                'created_at'    => now(),
    ...
    {
        $module = DB::table('modules')->where('module_slug', 'ramadan-packages')->first();
        if ($module) {
