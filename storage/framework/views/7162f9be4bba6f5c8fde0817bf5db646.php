<section class="flec item-center justify-center pt-14 pb-18">
    <div class="container mx-auto">
        <div class="mx-auto text-center">
            <h1>
                <span>How to Book </span><span class="text-mst">Your Dubai Tour?</span>
            </h1>
            <p class="mt-4">
                Book Dubai tours, desert safari deals, and yacht experiences in minutes with secure payment and instant
                confirmation.</p>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-4 gap-10 mt-14">
            
            <div class="bg-gray-50 p-8 rounded-xl border-1 border-gray-200 space-y-4 relative">
                <div class="rounded-full bg-gradient-to-r from-mst to-mst-dark px-4 pb-1 pt-0 font-heading italic
                text-md text-center w-fit text-white absolute -top-4 md:left-35 left-42">01
                </div>
                <img src="<?php echo e(asset('assets/images/process_arrow.png')); ?>"
                     class="absolute left-70 hidden md:block"
                     alt="img"> <img src="<?php echo e(asset('assets/images/icons/explore.svg')); ?>" class="mx-auto" alt="img">
                <h5 class="font-heading text-md font-semibold text-center mb-1">Explore & Select</h5>
                <p class="font-body text-xs text-center">
                    Browse top-rated Dubai tours, desert safari & yacht experiences.</p>
            </div>
            
            <div class="bg-gray-50 p-8 rounded-xl border-1 border-gray-200 space-y-4 relative">
                <div class="rounded-full bg-gradient-to-r from-mst to-mst-dark px-4 pb-1 pt-0 font-heading italic
                text-md text-center w-fit text-white absolute -top-4 md:left-35 left-42">02
                </div>
                <img src="<?php echo e(asset('assets/images/process_arrow.png')); ?>"
                     class="absolute left-70 hidden md:block"
                     alt="img"> <img src="<?php echo e(asset('assets/images/icons/enter.svg')); ?>" class="mx-auto" alt="img">
                <h5 class="font-heading text-md font-semibold text-center mb-1">Enter Details</h5>
                <p class="font-body text-xs text-center">Choose your date, guests & pickup location.</p>
            </div>
            
            <div class="bg-gray-50 p-8 rounded-xl border-1 border-gray-200 space-y-4 relative">
                <div class="rounded-full bg-gradient-to-r from-mst to-mst-dark px-4 pb-1 pt-0 font-heading italic
                text-md text-center w-fit text-white absolute -top-4 md:left-35 left-42">03
                </div>
                <img src="<?php echo e(asset('assets/images/process_arrow.png')); ?>"
                     class="absolute left-70 hidden md:block"
                     alt="img"> <img src="<?php echo e(asset('assets/images/icons/makepayment.svg')); ?>" class="mx-auto" alt="img">
                <h5 class="font-heading text-md font-semibold text-center mb-1">Make Payment</h5>
                <p class="font-body text-xs text-center">Secure payment with instant booking confirmation.</p>
            </div>
            
            <div class="bg-gray-50 p-8 rounded-xl border-1 border-gray-200 space-y-4 relative">
                <div class="rounded-full bg-gradient-to-r from-mst to-mst-dark px-4 pb-1 pt-0 font-heading italic
                text-md text-center w-fit text-white absolute -top-4 md:left-35 left-42">04
                </div>
                <img src="<?php echo e(asset('assets/images/icons/confirm.svg')); ?>" class="mx-auto" alt="img">
                <h5 class="font-heading text-md font-semibold text-center mb-1">Confirm & Enjoy</h5>
                <p class="font-body text-xs text-center">Get confirmation & enjoy hassle-free pickup and tour.</p>
            </div>
        </div>
    </div>
</section>
<?php /**PATH D:\laragon\www\mysaifco-laravel\resources\views/frontend/components/booking_process.blade.php ENDPATH**/ ?>