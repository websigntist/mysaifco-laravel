                                        ...
                                        <?php $__currentLoopData = $columns; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $col): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            ...
                                            <td class="capitalize">
                                                ...
                                                <?php elseif($col === 'image'): ?>
                                                    <div class="avatar avatar-md lightgallery">
                                                        <a href="<?php echo e($data->image ? asset('assets/images/'.$module .'/'. $data->image) : '#'); ?>">
                                                ...
                                                <?php elseif($col === 'friendly_url'): ?>
                                                    <code class="small"><?php echo e($data->friendly_url); ?></code>
                                                <?php elseif($col === 'description'): ?>
                                                    <?php echo e(Str::limit(strip_tags($data->description), 80)); ?>
                ...
                btn.addEventListener("click", async function () {
                    ...
                    try {
                        ...
                        if (!data || Object.keys(data).length === 0) {
                            contentArea.innerHTML = `<tr><td colspan="2" class="text-center text-warning">No data found.</td></tr>`;
                            modal.show();
                            return;
                        }
                        const imageUrl = data.image ? `/assets/images/<?php echo e($module); ?>/${data.image}` : "<?php echo e(imageNotFound()); ?>";
                        const rows = `
                            <tr><th><?php echo e(_label('ID')); ?></th><td>${data.id ?? '-'}</td></tr>
                            <tr><th>Name</th><td>${data.name ?? '-'}</td></tr>
                            <tr><th>Friendly URL</th><td><code>${data.friendly_url ?? '-'}</code></td></tr>
                            <tr><th><?php echo e(_label('description')); ?></th><td>${data.description ?? '-'}</td></tr>
                            <tr><th><?php echo e(_label('image')); ?></th><td><img src="${imageUrl}" alt="" style="max-height:80px" class="rounded"></td></tr>
                            <tr><th><?php echo e(_label('status')); ?></th><td><span class="badge ${data.status === 'Active' ? 'bg-label-success' : 'bg-label-danger'}">${data.status ? data.status : '-'}</span></td></tr>
...
<?php echo $__env->make('backend.layouts.master', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laragon\www\mysaifco-laravel\resources\views\backend\brands\listing.blade.php ENDPATH**/ ?>