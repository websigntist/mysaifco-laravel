                                            ...
                                            <td class="capitalize">
                                                ...
                                                <?php elseif($col === 'status'): ?>
                                                    <span id="statusLabel-<?php echo e($data->id); ?>"
                                                          class="badge <?php echo e($data->status === 'Replied' ? 'bg-label-success' : 'bg-label-danger'); ?>"><?php echo e(ucfirst($data->status)); ?>
            ...
            btn.addEventListener("click", async function () {
                ...
                try {
                    ...
                    const imageUrl = data.document
                        ? `/assets/documents/${data.document}`
                        : "<?php echo e(imageNotFound()); ?>";

                    // Build data rows
                    let rows = `
                        <tr><th><?php echo e(_label('ID')); ?></th><td>${data.id ?? '-'}</td></tr>
                        <tr><th><?php echo e(_label('first_name')); ?></th><td>${data.first_name ?? '-'}</td></tr>
                        <tr><th><?php echo e(_label('last_name')); ?></th><td>${data.last_name ?? '-'}</td></tr>
                        <tr><th><?php echo e(_label('email')); ?></th><td>${data.email ?? '-'}</td></tr>
                        <tr><th><?php echo e(_label('phone')); ?></th><td>${data.phone ?? '-'}</td></tr>
                        <tr><th><?php echo e(_label('message')); ?></th><td>${data.message ?? '-'}</td></tr>
                        <tr>
                            <th><?php echo e(_label('status')); ?></th>
                            <td>
                                <span class="badge ${data.status === 'Replied' ? 'bg-label-success' : 'bg-label-danger'}">
                    ...
                    if (data.document) {
                        rows += `
                            <tr>
                                <th><?php echo e(_label('document')); ?></th>
                                <td>
                                    <a href="${imageUrl}" download>
                                        <button type="button" class="btn btn-sm btn-primary mt-7 d-flex">
...
<?php echo $__env->make('backend.layouts.master', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laragon\www\mysaifco-laravel\resources\views\backend\inquiries\listing.blade.php ENDPATH**/ ?>